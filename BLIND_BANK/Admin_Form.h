		...
		else
		{
			...
			try
			{
				...
				//	polecenie->CommandText = "UPDATE `blind_bank_db`.`uzytkownicy` SET `grupy_idgrupy` = '" + id_grupy + "' WHERE `iduzytkownicy` = '" + id_rekordu + "';";
					...
					polecenie->CommandText = "INSERT INTO grupy_uzytkownikow (iduzytkownika, idgrupy, Numer_Indexu_uzytkownika, Numer_Grupy, Informacje) VALUES ('" + id_rekordu + "','" + id_grupy + "', (SELECT Unique_Index_Number FROM uzytkownicy WHERE iduzytkownicy = "+ id_rekordu +"), (SELECT Numer_Grupy FROM grupy WHERE idgrupy = "+ id_grupy +") ,(SELECT Informacje FROM grupy WHERE idgrupy = " + id_grupy + "));";
					polecenie->ExecuteNonQuery();
					transakcja->Commit();
